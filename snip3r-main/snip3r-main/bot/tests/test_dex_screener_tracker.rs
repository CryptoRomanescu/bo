...
use h_5n1p3r::oracle::{DexScreenerConfig, DexScreenerTracker, TrendingEvent, TrendingMetrics};
...
fn test_pump_dump_detection() {
    ...
    if let (
        TrendingEvent::Entry { rank: entry_rank },
        TrendingEvent::RankUp { old_rank, new_rank },
    ) = (entry_event, rank_up_event)
    {
    ...
    if let TrendingEvent::Exit { final_rank } = exit_event {
        assert!(
            final_rank > 50,
            "Exit from trending indicates potential dump"
        );
    }
}
...
fn test_momentum_score_factors() {
    ...
    assert!(
        (duration_score - 50.1).abs() < 0.2,
        "Duration score {} not close to 50",
        duration_score
    );
