...
fn analyze_scenario(
    ...
    println!("\n--- Bot Metrics ---");
    println!("Bot Transactions: {}/{} ({:.1}%)", 
             analysis.bot_metrics.bot_transaction_count,
             analysis.total_transactions,
             analysis.bot_percentage * 100.0);
    println!("Unique Bot Addresses: {}", analysis.bot_metrics.unique_bot_addresses);
    println!("Avg Bot Response Time: {:.1}ms", analysis.bot_metrics.avg_bot_response_time_ms);
    println!("Instant Tx %: {:.1}%", analysis.bot_metrics.instant_transaction_percentage * 100.0);
    println!("High-Frequency Bots: {}", analysis.bot_metrics.high_frequency_bots);
    
    println!("\n--- Bundle Metrics ---");
    println!("Bundle Count: {}", analysis.bundle_metrics.bundle_count);
    println!("Bundled Transactions: {}/{} ({:.1}%)",
             analysis.bundle_metrics.bundled_transaction_count,
             analysis.total_transactions,
             analysis.bundle_metrics.bundle_percentage * 100.0);
    println!("Max Bundle Size: {}", analysis.bundle_metrics.max_bundle_size);
    println!("Avg Bundle Size: {:.1}", analysis.bundle_metrics.avg_bundle_size);
    println!("Coordinated Bundles: {}", analysis.bundle_metrics.coordinated_bundle_count);
    
    println!("\n--- Health Indicators ---");
    println!("Organic Ratio: {:.1}%", analysis.organic_ratio * 100.0);
    println!("Manipulation Score: {:.2}", analysis.manipulation_score);
    println!("Suspicious Clusters: {}", analysis.suspicious_clusters.len());
...
async fn main() {
    ...
    println!("- Bot Timing Threshold: {}ms", config.bot_timing_threshold_ms);
    println!("- Bundle Time Window: {}ms", config.bundle_time_window_ms);
    println!("- Healthy Organic Threshold: {:.0}%", config.healthy_organic_threshold * 100.0);
    println!("- Avoid Bot Threshold: {:.0}%", config.avoid_bot_threshold * 100.0);
    ...
    analyze_scenario("Bundle-Coordinated Token", transactions, deploy_ts, &detector);
