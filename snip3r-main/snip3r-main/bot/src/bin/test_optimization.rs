...
use anyhow::Result;
use h_5n1p3r::oracle::{
    DecisionLedger, FeatureWeights, Outcome, PerformanceMonitor, ScoreThresholds, ScoredCandidate,
    StrategyOptimizer, TransactionRecord,
};
...
async fn main() -> Result<()> {
    ...
    let decision_ledger =
        DecisionLedger::new(decision_record_receiver, outcome_update_receiver).await?;
    ...
    tokio::spawn(async move {
        ...
        if let Ok(report) = performance_monitor.analyze_performance().await {
            info!(
                "Performance analysis results: PF={:.2}, WR={:.1}%",
    ...
    if let Some(optimized_params) = opt_params_receiver.recv().await {
        ...
        info!(
            "Original liquidity weight: {:.3}",
            FeatureWeights::default().liquidity
        );
        info!(
            "New liquidity weight: {:.3}",
            optimized_params.new_weights.liquidity
        );
        info!(
            "Improvement: {:.1}%",
            ((optimized_params.new_weights.liquidity / FeatureWeights::default().liquidity) - 1.0)
                * 100.0
        );
    } else {
