...
use crate::oracle::types_old::{
    Attribute, CreatorHoldings, HolderData, LiquidityPool, Metadata, PoolType, SocialActivity,
...
impl OracleDataSources {
    ...
    async fn fetch_token_data(&self, candidate: &PremintCandidate) -> Result<TokenData> {
        ...
        let creator_holdings = self
            .fetch_creator_holdings(candidate, &())
            .await
            .unwrap_or_default();
    ...
    async fn fetch_metadata_from_uri(&self, uri: &str) -> Result<Metadata> {
        ...
        let metadata: Metadata = response.json().await.context("Failed to parse metadata")?;
    ...
    async fn fetch_volume_data(
        ...
        debug!(
            "Volume data: {} transactions, growth rate: {:.2}x",
