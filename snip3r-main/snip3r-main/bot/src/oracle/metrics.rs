...
impl OracleMetricsCollector {
    ...
    fn register_standard_metrics(
        ...
        // Metacognition/Calibration Gauges
        let oracle_confidence_bias = Gauge::with_opts(opts!(
            "oracle_confidence_bias",
            "Confidence bias (positive=overconfident, negative=underconfident)"
        )).unwrap();
        registry.register(Box::new(oracle_confidence_bias.clone())).unwrap();
        gauges.insert("oracle_confidence_bias".to_string(), oracle_confidence_bias);

        let oracle_calibration_error = Gauge::with_opts(opts!(
            "oracle_calibration_error",
            "Mean absolute calibration error"
        )).unwrap();
        registry.register(Box::new(oracle_calibration_error.clone())).unwrap();
        gauges.insert("oracle_calibration_error".to_string(), oracle_calibration_error);

        let oracle_avg_predicted_confidence = Gauge::with_opts(opts!(
            "oracle_avg_predicted_confidence",
            "Average predicted confidence level"
        )).unwrap();
        registry.register(Box::new(oracle_avg_predicted_confidence.clone())).unwrap();
        gauges.insert("oracle_avg_predicted_confidence".to_string(), oracle_avg_predicted_confidence);

        let oracle_calibration_decision_count = Gauge::with_opts(opts!(
            "oracle_calibration_decision_count",
            "Number of decisions in calibration window"
        )).unwrap();
        registry.register(Box::new(oracle_calibration_decision_count.clone())).unwrap();
        gauges.insert("oracle_calibration_decision_count".to_string(), oracle_calibration_decision_count);
    ...
    /// Update calibration metrics from calibration stats
    pub async fn update_calibration_metrics(&self, stats: &crate::oracle::metacognition::CalibrationStats) {
        self.set_gauge("oracle_confidence_bias", stats.confidence_bias).await;
        self.set_gauge("oracle_calibration_error", stats.calibration_error).await;
        self.set_gauge("oracle_avg_predicted_confidence", stats.avg_predicted_confidence).await;
        self.set_gauge("oracle_calibration_decision_count", stats.decision_count as f64).await;
    }
    ...
    pub async fn start_metrics_server(
        ...
        let make_svc = make_service_fn(move |_conn| {
            ...
            async move {
                Ok::<_, Infallible>(service_fn(move |req| {
                    ...
                    async move { serve_metrics(req, registry, auth_token).await }
                }))
            }
        });
        ...
        if auth_token.is_some() {
            info!("Starting Prometheus metrics server on {} with authentication enabled", addr);
        } else {
            warn!("Starting Prometheus metrics server on {} WITHOUT authentication - NOT recommended for production", addr);
        }
    ...
    if let Some(ref expected_token) = auth_token {
        if let Some(auth_header) = req.headers().get("authorization") {
            if let Ok(auth_str) = auth_header.to_str() {
                ...
                let provided_token = auth_str.strip_prefix("Bearer ").unwrap_or(auth_str);
...
mod tests {
    ...
    #[tokio::test]
    async fn test_update_calibration_metrics() {
        use crate::oracle::metacognition::{ConfidenceCalibrator, DecisionOutcome};
        
        let collector = OracleMetricsCollector::new();
        
        // Create some calibration stats
        let mut calibrator = ConfidenceCalibrator::new();
        for i in 0..20 {
            let outcome = DecisionOutcome::new(0.80, 80, i < 10, 0.0, 1000 + i);
            calibrator.add_decision(outcome);
        }
        
        let stats = calibrator.get_stats();
        
        // Update metrics with calibration stats
        collector.update_calibration_metrics(&stats).await;
        
        let snapshot = collector.get_metrics_snapshot().await;
        
        // Verify all calibration metrics are set
        assert!(snapshot.gauges.contains_key("oracle_confidence_bias"));
        assert!(snapshot.gauges.contains_key("oracle_calibration_error"));
        assert!(snapshot.gauges.contains_key("oracle_avg_predicted_confidence"));
        assert!(snapshot.gauges.contains_key("oracle_calibration_decision_count"));
        
        // Verify values
        let bias = snapshot.gauges.get("oracle_confidence_bias").unwrap();
        assert!(*bias > 0.0, "Should detect overconfidence");
        
        let decision_count = snapshot.gauges.get("oracle_calibration_decision_count").unwrap();
        assert_eq!(*decision_count, 20.0);
    }
}