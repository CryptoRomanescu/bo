...
use crate::oracle::types::{
    FeatureWeights, OptimizedParameters, OptimizedParametersSender, Outcome,
    PerformanceReportReceiver, ScoreThresholds, TransactionRecord,
};
    ...
    pub async fn run(mut self) {
        ...
        while let Some(report) = self.report_receiver.recv().await {
            ...
            if report.profit_factor < 1.2 && report.total_trades_evaluated > 10 {
                warn!(
                    "Profit Factor is below threshold ({:.2}). Attempting to optimize strategy.",
    ...
    async fn find_optimizations(&mut self) -> Result<Option<OptimizedParameters>> {
        ...
        for trade in &losing_trades {
            for (feature, score) in &trade.scored_candidate.feature_scores {
                let entry = avg_scores_on_losses
                    .entry(feature.clone())
                    .or_insert((0.0, 0));
                entry.0 += score;
                entry.1 += 1;
