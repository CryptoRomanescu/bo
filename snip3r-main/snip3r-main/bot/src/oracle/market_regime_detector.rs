...
impl MarketRegimeDetector {
        ...
        if *current_regime_lock != new_regime {
            ...
            *current_regime_lock = new_regime;
            ...
            debug!(
                ...
                *current_regime_lock, sol_price, volatility, network_tps
            );
        }
    ...
    fn determine_regime(
        ...
        if tps > 3000.0 {
            debug!(
                "High TPS detected ({:.0}), classifying as HighCongestion",
                tps
            );
        ...
        if volatility_percent > 5.0 {
            debug!(
                "High volatility detected ({:.1}%), classifying as Choppy",
                volatility_percent
            );
