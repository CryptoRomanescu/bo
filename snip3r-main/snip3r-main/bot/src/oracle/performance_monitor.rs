...
use crate::oracle::types::{
    Outcome, PerformanceReport, PerformanceReportSender, TransactionRecord,
};
...
impl PerformanceMonitor {
    ...
    pub async fn run(self) {
        info!(
            "PerformanceMonitor is running. Analysis every {} minutes.",
        ...
        loop {
            ...
            match self.analyze_performance().await {
                Ok(report) => {
                    info!(
                        "Performance analysis complete. Profit Factor: {:.2}, Win Rate: {:.2}%",
    ...
    pub async fn analyze_performance(&self) -> Result<PerformanceReport> {
        let since_timestamp = (chrono::Utc::now()
            - chrono::Duration::hours(self.time_window_hours as i64))
        ...
        let closed_trades: Vec<_> = records
            .iter()
