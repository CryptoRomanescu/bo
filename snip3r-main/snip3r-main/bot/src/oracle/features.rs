...
use crate::oracle::{DexScreenerConfig, DexScreenerTracker};
...
pub struct OracleFeatureComputer {
    config: OracleConfig,
    dex_tracker: Option<Arc<DexScreenerTracker>>,
}

impl OracleFeatureComputer {
    /// Create a new feature computer with the given configuration.
    pub fn new(config: OracleConfig) -> Self {
        // Initialize DEX Screener tracker if API key is provided
        let dex_tracker = if config.dex_screener_api_key.is_some() {
            let dex_config = DexScreenerConfig {
                api_key: config.dex_screener_api_key.clone(),
                enable_tracking: true,
                ..DexScreenerConfig::default()
            };
            Some(Arc::new(DexScreenerTracker::new(dex_config)))
        } else {
            None
        };

        Self { config, dex_tracker }
    }
    ...
    pub async fn compute_all_features(
        ...
        // Compute DEX Screener trending features
        let (dex_trending_score, dex_momentum_score) = self.compute_dex_screener_scores(candidate).await?;
        scores.set(Feature::DexTrending, dex_trending_score);
        scores.set(Feature::DexMomentum, dex_momentum_score);
    ...
    /// Compute DEX Screener trending and momentum scores.
    /// Returns (trending_score, momentum_score) tuple.
    #[instrument(skip(self), fields(mint = %candidate.mint))]
    async fn compute_dex_screener_scores(&self, candidate: &PremintCandidate) -> Result<(f64, f64)> {
        // Return zero scores if DEX tracker is not initialized
        let tracker = match &self.dex_tracker {
            Some(t) => t,
            None => {
                debug!("DEX Screener tracker not initialized, returning zero scores");
                return Ok((0.0, 0.0));
            }
        };

        // Fetch trending metrics for the token
        let metrics = match tracker.get_trending_metrics(&candidate.mint).await {
            Ok(m) => m,
            Err(e) => {
                warn!("Failed to fetch DEX Screener metrics: {}", e);
                return Ok((0.0, 0.0));
            }
        };

        // Compute trending score based on:
        // 1. Is token trending (binary signal)
        // 2. Current rank (lower is better)
        // 3. Duration in trending (longer is better, up to a point)
        let trending_score = if metrics.is_trending {
            let mut score = 0.5; // Base score for being trending

            // Rank bonus: top 10 = +0.3, top 20 = +0.2, top 50 = +0.1
            ...
            // Duration bonus: sustained trending (5+ minutes) = +0.2
            if metrics.duration_in_trending >= 300 {
                score += 0.2;
            }
        ...
        // Compute momentum score based on:
        // 1. Velocity (rank change rate)
        // 2. Price correlation (trending + price increase = strong signal)
        // 3. Volume correlation (trending + volume increase = strong signal)
        let momentum_score = if metrics.is_trending {
            // Normalize momentum score (0-100) to 0-1
            let base_momentum = (metrics.momentum_score / 100.0).clamp(0.0, 1.0);

            // Correlation bonus: strong positive correlation = multiplier
            let correlation_bonus = (metrics.price_correlation + metrics.volume_correlation) / 2.0;
            
            // Combined score with correlation boost
            let combined = base_momentum * 0.7 + correlation_bonus * 0.3;
        ...
        debug!(
            "DEX Screener scores: trending={:.2} (is_trending={}, rank={:?}, duration={}s), momentum={:.2} (velocity={:.2}, price_corr={:.2}, vol_corr={:.2})",
            trending_score,
            metrics.is_trending,
            metrics.current_rank,
            metrics.duration_in_trending,
            momentum_score,
            metrics.velocity,
            metrics.price_correlation,
            metrics.volume_correlation
        );

        Ok((trending_score, momentum_score))
    }
}
